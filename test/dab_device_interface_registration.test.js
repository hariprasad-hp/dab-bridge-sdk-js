...
const { DabDeviceInterface } = await import("../src/interface/dab_device_interface.js");
...
describe("DabDeviceInterface DAB 2.1 registration", () => {
  ...
  test("registers all expected DAB 2.1 operation topics", async () => {
  ...
  test("reports DAB 2.1 version support", () => {
    const device = new MockDabDevice("device-1");
    expect(device.version()).toEqual({ status: 200, versions: ["2.0", "2.1"] });
  });

  test("applies SDK-level validation to registered handlers", async () => {
    const device = new MockDabDevice("device-1");
    await device.init("mqtt://broker");
    const registeredTopics = mqttInstances[0].handlers;

    const searchHandler = registeredTopics.get(`dab/device-1/${topics.CONTENT_SEARCH_TOPIC}`);
